...
- **摘要快速筛选**：`litrx abstract` 根据 `configs/questions/abstract.yaml` 中的自定义问题进行是/否判定和开放式问题回答，可选地对每个答案进行二次校验（通过配置项 `ENABLE_VERIFICATION` 或界面勾选“启用验证”切换），结果写入对应的 `<列名>_verified` 字段。图形界面提供 **“编辑问题”** 对话框可在运行中调整题目、只读日志区输出带 ✔/✖ 标记的模型摘要、**“中止任务”** 按钮随时停止处理，并可将结果导出为 CSV 或 Excel。
...
- **摘要筛选**
  ...
  ```
  修改 `configs/questions/` 下的文件可自定义筛选问题与列名。结果表格会为每个问题生成 `<列名>` 和 `<列名>_verified` 两列，后者指示 AI 回答是否得到标题或摘要支持（禁用验证时显示“未验证”）。当启用验证时，日志会在答案后附带 ✔/✖ 标记。
- **PDF 筛选**
  ```bash
